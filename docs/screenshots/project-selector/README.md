...
Reproduce with `cargo run -p kratos-ui --example project-selector-fixture` on a desktop (or Xvfb with a window manager). Open **All projects**, then type `anara`. Down and Return select the second matching project; the trigger shows `anara @ Build server` and its offline glyph.
...
Validation: `cargo build -p kratos`; `cargo build -p kratos-ui --example project-selector-fixture`; `cargo test -p kratos-ui --lib state::tests:: -- --test-threads=1` (54 passed); rustfmt checks on the changed Rust files; `git diff --check`.
