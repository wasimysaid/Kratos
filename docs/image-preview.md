...
cargo test --release --locked -p kratos-engine --lib --test workspace_files --test device_routing
```
...
No zui dependency changes are required. Kratos uses `on_pinch` and `on_scroll_wheel` from its existing pinned revision.
