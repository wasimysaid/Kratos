...
Desktop syntax highlighting lives in the pure `kratos-syntax` crate. It detects languages, runs pinned Tree-sitter grammars and queries, and returns sorted, non-overlapping UTF-8 byte spans relative to each source line. The UI resolves those neutral `HighlightKind` values through `Theme::syntax`; parser code never depends on GPUI or colors.
...
5. Run `cargo test -p kratos-syntax`, UI Markdown/Changes tests, the ignored diagnostic benchmark when parser cost changes, and the workspace checks.
