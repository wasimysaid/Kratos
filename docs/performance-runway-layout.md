...
Build each revision with `cargo build --release --locked -p kratos`, copy each
...
DISPLAY=:108 WAYLAND_DISPLAY= LP_NUM_THREADS=4 KRATOS_FRAME_STATS=0 \
  KRATOS_PROFILE_PSS=1 KRATOS_PROFILE_SUBMIT_UI=1 \
  KRATOS_PROFILE_PRE_IDLE_MS=10000 KRATOS_PROFILE_IDLE_MS=45000 \
  KRATOS_REPLAY_DELAY_MS=40 \
