...
for (const [platform, userAgent, file] of [
  ...
  for (const id of ['nav-download', 'hero-download', 'closing-download']) assert.equal(links[id].href, `https://kratos.sh/releases/kratos-0.2.67-${file}`);
  assert.equal(choices.length, 4);
  for (const choice of choices) assert.match(choice.href, /^https:\/\/kratos.sh\/releases\/kratos-0.2.67-/);
});
...
for (const [version, fail] of [['<script>alert(1)</script>', false], ['0.2.65', false], ['', true]]) test(`safe fallback: ${version}`, async () => {
  ...
  assert.equal(links['hero-download'].href, 'https://kratos.sh/releases/kratos-0.2.66-windows-x86_64.zip');
});
test('HTML retains all four explicit downloads without JavaScript', () => {
  for (const file of ['macos-arm64.dmg', 'windows-x86_64.zip', 'linux-x86_64.tar.gz', 'linux-aarch64.tar.gz']) assert.ok(html.includes(`https://kratos.sh/releases/kratos-0.2.66-${file}`));
