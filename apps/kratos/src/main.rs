//! kratos — headed by default; `kratos headless` runs the engine alone. Both start
//! local-only without credentials. Pairing and `kratos logout` select the
...
struct Cli {
    ...
    /// Open a Kratos conversation URL.
    #[arg(value_name = "URL")]
...
enum Command {
    ...
    Appshot,
    /// Manage `kratos headless` as a background service (launchd / systemd --user).
    Daemon {
        #[command(subcommand)]
...
#[derive(Subcommand)]
enum DaemonCommand {
    /// Install, enable, and start the service (captures KRATOS_* env).
    Install,
...
fn main() -> anyhow::Result<()> {
    ...
    if let Some(pid) = cli.wait_for_exit {
        kratos_update::windows::wait_for_exit(pid)?;
    }
    ...
    match cli.command {
        Some(Command::Headless) => {
            ...
            runtime.block_on(async {
                let engine = kratos_engine::Engine::new(engine_config_from_env());
                engine.run().await
            })
        }
        ...
        Some(Command::Appshot) => {
            kratos_ui::appshots::request_running_appshot(&engine_config_from_env().data_dir)
        ...
        None => {
            // Headed: the UI probes KRATOS_IPC_PORT and connects to a running
            // daemon, or embeds the engine in-process (ARCHITECTURE §1).
            kratos_ui::run_app(kratos_ui::UiConfig {
                data_dir: paths::data_dir(),
                ipc_port: std::env::var("KRATOS_IPC_PORT")
                    .ok()
                    .and_then(|p| p.parse().ok())
                    .unwrap_or(27654),
                default_harness: kratos_ui::HarnessId::ClaudeCode,
...
mod removed_command_tests {
    ...
    fn migrate_is_rejected_after_cli_removal() {
        let cli = Cli::try_parse_from(["kratos", "migrate"]).expect("positional parser");
...
fn engine_config_from_env() -> kratos_engine::EngineConfig {
    kratos_engine::EngineConfig {
        data_dir: paths::data_dir(),
        ipc_port: std::env::var("KRATOS_IPC_PORT")
            .ok()
            .and_then(|p| p.parse().ok())
...
/// `KRATOS_HARNESS` (kebab-case id) picks the default harness for chats without a
/// config row — `mock` powers the e2e smoke; default `claude-code`.
fn harness_from_env() -> kratos_engine::HarnessId {
    match std::env::var("KRATOS_HARNESS").as_deref().map(str::trim) {
        Ok("mock") => kratos_engine::HarnessId::Mock,
        Ok("codex") => kratos_engine::HarnessId::Codex,
        Ok("cursor") => kratos_engine::HarnessId::Cursor,
        Ok("devin") => kratos_engine::HarnessId::Devin,
        Ok("grok") => kratos_engine::HarnessId::Grok,
        Ok("hermes") => kratos_engine::HarnessId::Hermes,
        Ok("pi") => kratos_engine::HarnessId::Pi,
        Ok("mimir") => kratos_engine::HarnessId::Mimir,
        _ => kratos_engine::HarnessId::ClaudeCode,
    }
}

/// `kratos sync`: dial the running engine's IPC and print per-room sync state.
...
async fn sync_cli(ipc_port: u16) -> anyhow::Result<()> {
    let client = kratos_rpc::connect_ws(&format!("ws://127.0.0.1:{ipc_port}"))
        .await
        .map_err(|e| {
            anyhow::anyhow!("no engine listening on 127.0.0.1:{ipc_port} ({e}) — is kratos running?")
        })?;
    let status = client
        .call(kratos_rpc::methods::SYNC_STATUS, serde_json::json!({}))
        .await
...
/// `{data_dir}/logs/kratos-{mode}.log`, previous launch preserved as `.old`.
...
/// locked logs to `kratos-{mode}.{pid}.log` instead; the next lock-holding
...
fn open_log_file_in(dir: &std::path::Path, mode: &str) -> Option<std::fs::File> {
    ...
    let path = dir.join(format!("kratos-{mode}.log"));
    #[cfg(unix)]
    {
        ...
        if rc != 0 {
            ...
            return std::fs::File::create(
                dir.join(format!("kratos-{mode}.{}.log", std::process::id())),
            )
            .ok();
        }
        ...
        drop(existing);
        if preexisting {
            let _ = std::fs::rename(&path, dir.join(format!("kratos-{mode}.log.old")));
        }
...
mod log_file_tests {
    ...
    fn second_launch_never_rotates_a_live_processes_log() {
        ...
        assert!(dir.join("kratos-headed.log").is_file());
        ...
        let pid_path = dir.join(format!("kratos-headed.{}.log", std::process::id()));
        assert!(pid_path.is_file(), "expected pid-suffixed overflow log");
        assert!(
            !dir.join("kratos-headed.log.old").exists(),
        ...
        assert!(
            dir.join("kratos-headed.log.old").is_file(),
            "rotation resumes"
        );
        drop(third);
    }
...
/// Delete `kratos-{mode}.{pid}.log` overflow files older than a week — they
