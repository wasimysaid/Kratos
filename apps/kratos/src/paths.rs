...
fn resolve_data_dir(mut env: impl FnMut(&str) -> Option<OsString>) -> PathBuf {
    if let Some(dir) = env("KRATOS_DATA_DIR") {
    ...
    #[cfg(windows)]
    {
        ...
        let local = env("LOCALAPPDATA")
            ...
            .expect("LOCALAPPDATA and USERPROFILE not set; set KRATOS_DATA_DIR");
        local.join("Kratos")
    }
    #[cfg(not(windows))]
    {
        let home = PathBuf::from(env("HOME").expect("HOME not set"));
        let dir = home.join(".kratos");
...
mod tests {
    ...
    fn explicit_data_dir_needs_no_home() {
        assert_eq!(
            resolve(&[("KRATOS_DATA_DIR", "custom data")]),
    ...
    fn explorer_launch_without_home_uses_local_app_data() {
        assert_eq!(
            ...
            PathBuf::from(r"C:\Users\Test User\AppData\Local\Kratos"),
        );
    }
    ...
    fn windows_profile_fallback_handles_unicode_and_apostrophes() {
        assert_eq!(
            ...
            PathBuf::from(r"C:\Users\O'Brien 日本語\AppData\Local\Kratos"),
        );
    }
