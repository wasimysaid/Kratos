//! `kratos daemon …` — install/manage `kratos headless` as a background service:
...
//! selected environment captured at install time. Peer connectivity and trusted
//! identity are loaded from the private installation profile, never environment
//! bearer tokens. A fresh service stays local; `kratos peer init` or `kratos pair`
//! followed by a service restart enables the paired profile.
...
const LAUNCHD_LABEL: &str = "sh.kratos.app";
/// Same unit name the checksummed release installer (`scripts/install.sh`) writes, so
/// `kratos daemon …` manages that installation rather than a competing copy.
const SYSTEMD_UNIT: &str = "kratos.service";
...
/// default PATH won't find); the `KRATOS_*`/logging vars only when set.
const CAPTURED_ENV: &[&str] = &[
    "PATH",
    "KRATOS_DATA_DIR",
    "KRATOS_TAILCAT_ADAPTER",
    "KRATOS_RELEASES_URL",
    "KRATOS_AUTO_UPDATE",
    "KRATOS_IPC_PORT",
    "KRATOS_HARNESS",
...
pub fn install(data_dir: &Path) -> anyhow::Result<()> {
    let exe = std::env::current_exe().context("resolving the kratos executable path")?;
    ...
    if cfg!(target_os = "macos") {
        ...
        bail!("kratos daemon is only supported on macOS (launchd) and Linux (systemd)");
    }
    println!(
        "Without a paired profile the engine stays local-only; initialize or pair a peer, then restart to enable sync."
    );
    println!(
        "Logs: {}",
        if cfg!(target_os = "macos") {
...
pub fn start() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        let plist = launchd_plist_path()?;
        if !plist.exists() {
            bail!("not installed — run `kratos daemon install` first");
        }
        ...
        if !output.status.success() {
            println!(
                ...
                if launchd_plist_path()?.exists() {
                    " (installed — `kratos daemon start`)"
                } else {
                    " (not installed — `kratos daemon install`)"
                }
            );
            return Ok(());
        }
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    let mut unit = String::from(
        "[Unit]\nDescription=Kratos headless engine\nAfter=network-online.target\nStartLimitIntervalSec=60\nStartLimitBurst=5\n\n[Service]\n",
    );
...
/// The ExecStart binary path. An exe under `~/.kratos/app/` came from the
...
fn exec_path_for(exe: &Path, home: Option<&Path>) -> String {
    let installed = home
        .map(|home| home.join(".kratos/app"))
        .is_some_and(|app_root| exe.starts_with(app_root));
    if installed {
        "%h/.kratos/app/current/kratos".to_string()
    } else {
        format!("{}", exe.display())
    }
}
...
mod tests {
    ...
    fn systemd_unit_shape() {
        let unit = render_systemd_unit(
            Path::new("/usr/local/bin/kratos"),
            &[
                ("PATH".into(), "/usr/bin:/bin".into()),
                (
                    "KRATOS_TAILCAT_ADAPTER".into(),
                    "/opt/kratos/kratos-tailcat".into(),
                ),
                ("RUST_LOG".into(), "info,kratos=\"debug\"".into()),
            ],
        );
        assert!(unit.contains("ExecStart=/usr/local/bin/kratos headless\n"));
        assert!(unit.contains("Environment=\"PATH=/usr/bin:/bin\"\n"));
        assert!(unit.contains("Environment=\"KRATOS_TAILCAT_ADAPTER=/opt/kratos/kratos-tailcat\"\n"));
        // Inner quotes escaped so systemd re-parses the value verbatim.
        assert!(unit.contains("Environment=\"RUST_LOG=info,kratos=\\\"debug\\\"\"\n"));
        ...
        assert!(unit.contains("EnvironmentFile=-%h/.kratos/env"));
    ...
    fn curl_installer_always_starts_the_local_capable_service() {
        ...
        let installer = include_str!("../../../scripts/install.sh").replace("\r\n", "\n");
        ...
        assert!(installer.contains("systemctl --user enable kratos"));
        assert!(installer.contains("systemctl --user restart kratos"));
    }
    ...
    fn installed_exe_uses_the_current_symlink() {
        ...
        assert_eq!(
            exec_path_for(
                Path::new("/home/u/.kratos/app/0.3.0/kratos"),
                Some(Path::new("/home/u")),
            ),
            "%h/.kratos/app/current/kratos"
        );
        // Source build: literal path.
        assert_eq!(
            exec_path_for(
                Path::new("/src/target/debug/kratos"),
                Some(Path::new("/home/u"))
            ),
            "/src/target/debug/kratos"
        );
    }
    ...
    fn launchd_plist_shape() {
        let plist = render_launchd_plist(
            Path::new("/Users/x/kratos & co/kratos"),
            &[(
                "KRATOS_TAILCAT_ADAPTER".into(),
                "/Users/x/adapter?a=1&b=2".into(),
            )],
            Path::new("/Users/x/.kratos/daemon.log"),
        );
        assert!(plist.contains("<key>Label</key><string>sh.kratos.app</string>"));
        // XML-escaped exe path and env value.
        assert!(plist.contains("<string>/Users/x/kratos &amp; co/kratos</string>"));
        assert!(plist.contains("<string>/Users/x/adapter?a=1&amp;b=2</string>"));
        assert!(plist.contains("<string>headless</string>"));
        assert!(plist.contains("<key>SuccessfulExit</key><false/>"));
        assert!(
            plist.contains("<key>StandardOutPath</key><string>/Users/x/.kratos/daemon.log</string>")
        );
    }
}
