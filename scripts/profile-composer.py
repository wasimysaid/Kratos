...
name = out.name
binary = out / 'profiled-kratos'
shutil.copy2(args.binary, binary)
settings = json.loads(args.settings.read_text())
(out / 'ui-settings.json').write_text(json.dumps(settings))
env = os.environ | {'DISPLAY': args.display, 'WAYLAND_DISPLAY': '', 'KRATOS_IPC_PORT': args.ipc_port, 'KRATOS_HARNESS': 'mock', 'KRATOS_DATA_DIR': str(out), 'RUST_LOG': 'warn', 'LP_NUM_THREADS': '4'}
...
try:
    time.sleep(2)
    ids = subprocess.check_output(['xdotool', 'search', '--class', 'kratos'], env=env, text=True).splitlines()
    wid = ids[-1]
