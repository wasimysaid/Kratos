...
public static class KratosClientCapture {
...
[uint32]$owner = 0
[void][KratosClientCapture]::GetWindowThreadProcessId($hwnd, [ref]$owner)
...
$previousDpi = [KratosClientCapture]::SetThreadDpiAwarenessContext([IntPtr]::new(-4))
$bitmap = $null
try {
    $rect = [KratosClientCapture+Rect]::new()
    if (-not [KratosClientCapture]::GetClientRect($hwnd, [ref]$rect)) { throw 'GetClientRect failed' }
    $dpi = [KratosClientCapture]::GetDpiForWindow($hwnd)
    ...
    try {
        ...
        if (-not [KratosClientCapture]::PrintWindow($hwnd, $dc, 3)) { throw 'PrintWindow failed' }
    ...
    if ($previousDpi -ne [IntPtr]::Zero) { [void][KratosClientCapture]::SetThreadDpiAwarenessContext($previousDpi) }
}
