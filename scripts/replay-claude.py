#!/usr/bin/env python3
"""Replay text/reasoning from a real Kratos Claude journal at a fixed cadence.

Set CLAUDE_CODE_EXECUTABLE to this file and KRATOS_REPLAY_JOURNAL to the JSONL
...
KRATOS_REPLAY_DELAY_MS defaults to 40; KRATOS_REPLAY_REPEAT defaults to 1.
...
with open(os.environ["KRATOS_REPLAY_JOURNAL"]) as source:
...
delay = float(os.environ.get("KRATOS_REPLAY_DELAY_MS", "40")) / 1000
repeats = int(os.environ.get("KRATOS_REPLAY_REPEAT", "1"))
if repeats < 1:
    raise ValueError("KRATOS_REPLAY_REPEAT must be positive")
