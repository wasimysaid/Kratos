...
param(
    [string]$Exe = (Join-Path $PSScriptRoot '../target/release/kratos.exe'),
    [int]$Runs = 2
)
...
if (-not ('KratosWindowProbe' -as [type])) {
Add-Type @'
using System;
using System.Text;
using System.Runtime.InteropServices;
public static class KratosWindowProbe {
...
$overrides = @{
    ...
    KRATOS_DATA_DIR = (Join-Path $root 'Kratos')
    KRATOS_EDGE_TOKEN = $null; KRATOS_IPC_PORT = '0'
    KRATOS_EDGE_URL = 'http://127.0.0.1:1'; KRATOS_ORG_ID = $null
    KRATOS_HARNESS = 'mock'; RUST_BACKTRACE = '1'
}
$saved = @{}
...
try {
    ...
    foreach ($run in 1..$Runs) {
        ...
        try {
            ...
            [void][KratosWindowProbe]::GetClassName($hwnd, $class, $class.Capacity)
            [uint32]$owner = 0
            [void][KratosWindowProbe]::GetWindowThreadProcessId($hwnd, [ref]$owner)
            ...
            $device = (Get-Content -LiteralPath (Join-Path $root 'Kratos/device-id') -Raw).Trim()
