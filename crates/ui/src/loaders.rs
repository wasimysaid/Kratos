//! Loaders: the kratos pulse loader, the gradient matrix spinner, and the boot
...
use crate::motion::{self, GRADIENT_SPIN, PULSE_STAGGER, SPLASH_OUT, KRATOS_PULSE};
use crate::theme::{GlyphPalette, Theme};

// Shared with the terminal viewport (`kratos_proto::motion`) so both animate the
// same loaders from the same numbers.
pub use kratos_proto::motion::{
    MARK_CELLS, MARK_SPREAD, MATRIX_SIDE, KRATOS_CELLS, mark_cell_stagger,
};

/// The animated kratos mark (kratos-loader.tsx `KratosLoader`): the full logo
        ...
        .children(MARK_CELLS.iter().map(move |&(x, y)| {
            ...
            div()
                ...
                .child({
                    ...
                    // the stagger ADDS phase (kratos-loader.tsx delayFor).
...
pub fn kratos_loader(
    ...
    div()
        ...
        .items_center()
        .gap(px(slot / 2.0))
        .children((0..KRATOS_CELLS).map(move |i| {
...
pub use kratos_proto::motion::{GSPIN_DIM, GSPIN_ROW_TINTS};

/// The gradient matrix spinner (WorkingIndicator), ported from kratos's
    ...
    let content = div()
        ...
        .child(
            div()
                ...
                .child(SharedString::from("Setting up Kratos environment")),
        );
    if fading {
...
const _: () = {
    ...
    assert!(KRATOS_PULSE.duration_ms == 2400);
