...
impl AppshotCapabilities {
    pub fn setup_description(self) -> &'static str {
        match self.platform {
            ...
            AppshotPlatform::LinuxWayland => {
                "Your desktop owns capture and shortcut consent. Kratos checks each portal capability separately and explains any required fallback."
            }
            AppshotPlatform::LinuxX11 => {
                "X11 normally needs no capture permission. Kratos prefers an active-window screenshot portal when available and otherwise uses native X11 capture."
            }
    ...
    pub fn shortcut_description(self) -> &'static str {
        match self.platform {
            ...
            AppshotPlatform::LinuxWayland => {
                "Bind `kratos appshot` in your desktop's Keyboard Shortcuts settings."
            }
    ...
    pub fn capture_description(self) -> &'static str {
        match (self.platform, self.target) {
            (AppshotPlatform::MacOs, _) => {
                "Screen Recording lets Kratos capture the frontmost window. macOS may request one restart."
            }
            (AppshotPlatform::LinuxX11, _) => {
                "Kratos uses native X11 capture when active-window portal capture is unavailable. Obscured or protected windows may be incomplete."
            }
...
pub fn validate_capture_dimensions(width: u32, height: u32) -> Result<usize, CaptureError> {
    ...
    if pixels > MAX_CAPTURE_PIXELS || rgba_bytes > MAX_CAPTURE_RGBA_BYTES {
        return Err(CaptureError::CaptureFailed(format!(
            "The captured window ({width}×{height}) exceeds Kratos's capture budget."
        )));
    }
...
fn trim_appshot_padding(bytes: &[u8]) -> Result<Option<Vec<u8>>, CaptureError> {
    ...
    if bytes.len() as u64 > crate::attachments::MAX_ATTACHMENT_BYTES {
        return Err(CaptureError::CaptureFailed(
            "The captured window is larger than Kratos's 24 MB image limit.".into(),
        ));
    }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::PermissionRequired => f.write_str(
                "Window capture permission is required. Open Kratos Settings → Appshots for the platform-specific recovery step.",
            ),
...
/// socket used by `kratos appshot` when the desktop owns shortcut setup.
...
/// Ask the running headed Kratos process to capture an Appshot. Linux desktop
/// environments that do not implement the Global Shortcuts portal can bind
/// `kratos appshot` in their native Keyboard Shortcuts settings.
#[cfg(target_os = "linux")]
