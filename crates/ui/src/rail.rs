...
use kratos_doc::{MessagePart, MessageRole, SessionMessageEntry};
...
/// `KRATOS_SCROLL_TRACE=1` logs per-frame glide positions at `warn` level —
/// the smoothness measurement knob (same family as `KRATOS_FRAME_STATS`).
fn scroll_trace_enabled() -> bool {
    static ENABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ENABLED.get_or_init(|| {
        std::env::var("KRATOS_SCROLL_TRACE").is_ok_and(|v| !v.is_empty() && v != "0")
    })
}
