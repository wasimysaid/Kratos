...
//! account-shaped loading skeletons. Kratos retargets devices from the settings
...
/// Compact absolute reset moment (kratos settings.agents.tsx `formatReset`):
...
/// in the empty-state copy, kratos settings.agents.tsx `PROVIDERS`).
...
impl LoginFlow {
    /// Dialog title (kratos: "Add Claude account" / "Add Codex account").
...
pub struct AccountsPage {
    ...
    /// Retargeted by the page-header device switcher (kratos parity: the
...
impl AccountsPage {
    ...
    /// The page-header device switcher (kratos device-switcher.tsx): a quiet
    ...
    fn render_device_switcher(&mut self, theme: &Theme, cx: &mut Context<Self>) -> AnyElement {
        ...
        // Stable row order (registration time, then id) — kratos's switcher
    ...
    /// One usage window (kratos settings.agents.tsx `UsageMeter`): label ·
    /// 5px rounded-full bar (indigo → amber ≥80% → red ≥95%) · "NN% used" ·
    /// quiet reset time.
    fn render_usage_meter(
        &self,
        window: &kratos_proto::AgentUsageWindow,
        theme: &Theme,
        now: DateTime<Utc>,
    ) -> AnyElement {
    ...
    /// One account row (kratos settings.agents.tsx `AccountRow`): initial
    ...
    fn render_account_row(
        ...
        // Actions only on INACTIVE accounts (kratos `{!account.active && …}`):
            ...
            .child(
                div()
                    ...
                    .map(|el| {
                        ...
                        // (kratos: `usage ? meters : "Usage unavailable"…`).
    ...
    fn render_login_dialog(
        ...
        let url_link =
            |id: &'static str, label: &'static str, url: &str, cx: &mut Context<Self>| {
                ...
                // "Reopen the …" text link (kratos: `text-[12px]
                ...
                let body = match harness {
                    HarnessId::Cursor => {
                        "Finish signing in to Cursor in your browser. This mints a \
                         kratos-named API key you can revoke any time from Cursor's \
    ...
    /// A ghost account row (kratos settings.agents.tsx `SkeletonRow`): avatar,
    ...
    fn render_skeleton_row(
        ...
        use crate::motion;
        let delta = motion::pulse_delta(&motion::KRATOS_PULSE, cx.entity_id(), cx);
...
impl Render for AccountsPage {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Brand mark inside a 24px centered box (kratos: `grid size-6
        ...
        // One section per provider (kratos settings.agents.tsx `ProviderSection`):
            ...
            Loadable::Ready(snapshot) => {
                ...
                PROVIDERS
                    ...
                    .map(|(harness, name, cli)| {
                        ...
                        // EVERY warning renders its own strip (kratos maps them).
                    ...
                    .child(
                        widgets::page_column()
                            .child(
                                div()
                                    ...
                                    .child(
                                        ...
                                        // dimmed while a refresh is in flight (kratos
                                        // `disabled:opacity-50`).
                                        widgets::ghost_action(&theme)
                                            .id("accounts-refresh")
                                            .flex_none()
                            ...
                            .child(widgets::page_subtitle(
                                &theme,
                                "The Claude Code, Codex, and Cursor logins on this device. Kratos \
                            ...
                            .children(sections)
                            // Footer note (kratos: `mt-6 text-[12px] leading-relaxed
...
mod tests {
    ...
    #[test]
    fn usage_thresholds_match_kratos() {
