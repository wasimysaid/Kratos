...
//! lightness steps; the same scale kratos's Tailwind theme used) into gpui [`Hsla`].
...
use kratos_syntax::HighlightKind;
use kratos_theme::{
...
pub enum AccentColor {
    /// The exact upstream Kratos indigo.
    #[default]
...
impl AccentColor {
    ...
    fn tokens(self, appearance: Appearance) -> AccentTokens {
        ...
        let (primary, strong) = match (self, appearance) {
            (Self::Kratos, Appearance::Dark) => {
                (oklch(0.673, 0.182, 276.935), oklch(0.585, 0.233, 277.117))
            }
            (Self::Kratos, Appearance::Light) => {
...
impl From<AccentColor> for AccentPreset {
    fn from(value: AccentColor) -> Self {
        match value {
            AccentColor::Kratos => Self::Kratos,
...
impl From<AccentPreset> for AccentColor {
    fn from(value: AccentPreset) -> Self {
        match value {
            AccentPreset::Kratos => Self::Kratos,
...
fn model_appearance(appearance: kratos_theme::Appearance) -> Appearance {
    match appearance {
        kratos_theme::Appearance::Dark => Appearance::Dark,
        kratos_theme::Appearance::Light => Appearance::Light,
    }
}
...
impl TerminalColors {
    ...
    fn kratos(appearance: Appearance) -> Self {
        let id = match appearance {
            Appearance::Dark => "kratos-dark",
            Appearance::Light => "kratos-light",
        };
        let registry = ThemeRegistry::active();
        Self::from_variant(registry.variant(id).expect("Kratos terminal palette exists"))
    }
}
...
impl Theme {
    ...
    /// Main-panel header height (kratos `h-11`) — in-card headers (changes pane).
    ...
    /// Reserved status strip under the content outlet (kratos `h-6`) — the
    ...
    /// fixed Kratos grey would erase that palette's identity.
    ...
    /// Increase tint coverage only as far as needed for Kratos's shared text
    ...
    /// theme, so forcing frost does not reintroduce Kratos's neutral hover.
    ...
    pub fn dark_with_accent(accent_color: AccentColor) -> Self {
        ...
        Self {
            ...
            variant_id: "kratos-dark".into(),
            family_id: "kratos".into(),
            ...
            terminal: TerminalColors::kratos(Appearance::Dark),
    ...
    pub fn light_with_accent(accent_color: AccentColor) -> Self {
        ...
        Self {
            ...
            terminal: TerminalColors::kratos(Appearance::Light),
            font_sans: "Geist".into(),
            font_sans_fixed: "Geist".into(),
            font_mono: "Geist Mono".into(),
            font_terminal: "Geist Mono".into(),
    ...
    pub fn for_selection(
        ...
        let variant = registry
            ...
            .expect("the built-in registry contains both Kratos appearances");
    ...
    pub(crate) fn from_variant(
        ...
        let accent_color = match accent_selection {
            AccentSelection::ThemeDefault => AccentColor::Kratos,
...
mod tests {
    ...
    fn neutral_950_is_0a0a0a() {
        // oklch(0.145 0 0) is Tailwind neutral-950, kratos's app background.
    ...
    #[test]
    fn kratos_accent_is_the_exact_upstream_default() {
        let dark = Theme::dark();
        let light = Theme::light();
        assert_eq!(dark.accent_color, AccentColor::Kratos);
    ...
    fn theme_recommendation_and_surface_override_resolve_independently() {
        ...
        let kratos = Theme::dark();
        assert_ne!(catppuccin.surface, kratos.surface);
        ...
        let opaque_kratos = Theme::for_selection(
            Appearance::Dark,
            "kratos-dark",
            AccentSelection::ThemeDefault,
            SurfacePreference::Opaque,
        );
        assert_eq!(opaque_kratos.surface_treatment, SurfaceTreatment::Opaque);
        assert_eq!(opaque_kratos.glass(), opaque_kratos.surface);
    }
    ...
    #[test]
    fn layout_numbers_match_kratos() {
