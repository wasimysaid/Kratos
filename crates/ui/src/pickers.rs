...
use kratos_engine::registry::HarnessDescriptor;
use kratos_proto::{
...
/// Dev/testing knob: `KRATOS_SLOW_CATALOG_MS=<ms>` delays every harness and
...
/// (headless-rig captures; same family as `KRATOS_OPEN_PICKER`).
fn slow_catalog_delay() -> Option<std::time::Duration> {
    std::env::var("KRATOS_SLOW_CATALOG_MS")
        .ok()
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (kratos mints a `kratos/<name>`
...
/// lead with the flagship — kratos's `pickDefaultModel` Opus preference maps to
/// the same row here).
pub fn default_model(models: &[Model]) -> Option<&Model> {
    models.first()
}

/// A model's default reasoning: X-High when the ladder offers it (kratos
...
/// or foreign level — kratos use-run-config.ts's derived-model discipline).
...
pub fn browser_rows(listing: &FolderListing) -> Vec<&kratos_proto::FolderEntry> {
...
pub struct Pickers {
    ...
    config: DraftConfig,
    /// Sticky last-used picks (kratos `kratos.composer.defaults:v1`): seeds the
    ...
    focus: FocusHandle,
    /// `KRATOS_OPEN_PICKER` boot: keep claiming focus until it sticks, so
...
impl Pickers {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        // Dev/testing knob: `KRATOS_OPEN_PICKER=model|traits|repo|branch` boots
        // with that popover open — synthetic input can't reach the app on
        // headless compositors, so captures need a data-side path.
        let boot_open = match std::env::var("KRATOS_OPEN_PICKER").ok().as_deref() {
    ...
    fn effective_harness(&self, cx: &App) -> Option<HarnessId> {
        ...
        // model (it stays available under `KRATOS_HARNESS=mock`).
        self.harnesses
            .ready()
    ...
    /// Capture knob (`KRATOS_OPEN_DIALOG=model`): open the combined
    ...
    fn device_rows(&self, cx: &App) -> Vec<kratos_proto::Device> {
        let state = self.state.read(cx);
        let local = state.local_device_id.clone();
        let mut devices: Vec<kratos_proto::Device> = state.devices.clone();
    ...
    fn filtered_device_rows(&self, cx: &App) -> Vec<kratos_proto::Device> {
        let query = self.search.read(cx).text().to_string();
    ...
    fn trigger_chip(
        ...
        // Ghost pill (kratos composer/styles.tsx `pill`): `h-8 rounded-lg px-2.5
        ...
        div()
            ...
            // kratos composer/styles.tsx `pill`: `transition-colors` — the wash
    ...
    fn popover_frame(&self, width: f32, content: AnyElement, cx: &mut Context<Self>) -> AnyElement {
        ...
        popover::popover_card(&theme)
            .w(px(width))
            // kratos caps its tallest picker at min(640px, 75vh).
            .max_h(px(640.0))
    ...
    /// picker's rail + list panes bleed to the card edge (kratos
    ...
    /// The combined harness + model switcher (kratos harness-model-picker.tsx):
...
pub(crate) fn normalize_model_rows(harness: HarnessId, models: Vec<Model>) -> Vec<Model> {
    ...
    let catalog = match harness {
        HarnessId::ClaudeCode => kratos_harness::claude::catalog::static_models(),
        _ => Vec::new(),
    };
        ...
        .filter_map(|mut model| {
            ...
            if let Some(base) = strip_1m(&model.id.clone()) {
                ...
                if !model.options.iter().any(|o| o.id == "contextWindow") {
                    model.options.push(kratos_proto::ModelOption {
                        id: "contextWindow".into(),
                        label: "Context Window".into(),
                        choices: vec![
                            kratos_proto::ModelOptionChoice {
                                id: "200k".into(),
                                label: "200K".into(),
                            },
                            kratos_proto::ModelOptionChoice {
...
/// `KRATOS_HARNESS=mock` (the e2e/dev rig) opts the mock harness into the UI;
/// production launches never set it, so the mock never surfaces there.
fn mock_harness_enabled() -> bool {
    std::env::var("KRATOS_HARNESS")
        .ok()
        .as_deref()
        .map(str::trim)
        == Some("mock")
...
/// `KRATOS_HARNESS=mock` shows it; otherwise it only remains when it's
...
fn offered_harnesses_impl(list: &[HarnessDescriptor], allow_mock: bool) -> Vec<HarnessDescriptor> {
    visible_harnesses_impl(list, allow_mock)
        ...
        .filter(|d| {
            d.installed
                && (kratos_engine::registry::descriptor_enabled(d)
...
impl Render for Pickers {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let theme = Theme::of(cx).clone();
        // A KRATOS_OPEN_PICKER popover never went through `toggle`, so claim
        ...
        // A popover opened data-side (KRATOS_OPEN_PICKER) never went through
        ...
        // Chip shows the model's display name alone (kratos `modelText`); the
        ...
        // attach + send after this element (kratos composer-actions.tsx
        // arrangement).
        let left = div()
            .flex()
            .flex_row()
...
mod tests {
    use super::*;
    use kratos_proto::{FolderEntry, Model, ModelOption, ModelOptionChoice};
    ...
    fn mimir_identity_uses_its_own_mark_and_standard_catalog_gates() {
        ...
        let mut descriptor = HarnessDescriptor {
            ...
            steering_mode: kratos_proto::SteeringMode::TurnBoundary,
    ...
    fn descriptor(id: HarnessId, name: &str) -> HarnessDescriptor {
        HarnessDescriptor {
            ...
            enabled: Some(true),
            reasoning_levels: Vec::new(),
            steering_mode: kratos_proto::SteeringMode::StepBoundary,
    ...
    fn completion_prefix_lengths() {
        ...
        assert_eq!(completion_prefix_len("kratos", "kratos"), Some(6));
        assert_eq!(completion_prefix_len("kratos", ""), Some(0));
        assert_eq!(completion_prefix_len("kratos", "dev"), None);
    ...
    fn browser_navigation_reducer() {
        ...
        assert_eq!(browser_rows(&listing)[1].name, "kratos");
    }
    ...
    fn mock_harness_hidden_unless_alone() {
        ...
        // …and opted back in by KRATOS_HARNESS=mock (the e2e rig).
