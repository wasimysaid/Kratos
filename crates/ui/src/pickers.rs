...
impl Pickers {
    ...
    fn apply_model_catalog(
        ...
        let catalog_ready = matches!(self.models.get(&harness), Some(Loadable::Ready(_)));
        if harness == HarnessId::Mimir {
            if catalog_ready {
                self.mimir_verified_model = self.model_catalog_selection(harness, cx);
                self.ensure_models(harness, false, cx);
            } else {
                // Do not turn a failed detail lookup into an automatic retry loop.
                self.mimir_model_selection = self.model_catalog_selection(harness, cx);
            }
        }
        self.catalog_rev += 1;
