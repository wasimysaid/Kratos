...
/// The app gate (kratos's App.tsx phases). Pure.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum GatePhase {
...
/// port of kratos's `formatTimeAgo`.
...
/// Session-row sub-line, "project · branch" (kratos `chatLocation`): the repo
...
/// Per-kind chip label + one-line detail. Labels match kratos's `describeTool`
...
pub fn tool_group_summary(tools: &[(crate::ToolCall, bool)]) -> String {
    ...
    // Capitalize the first segment only (kratos's style).
...
/// read the same on every surface. `kratos-ui` has the oklch→sRGB math.
pub mod dot {
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (the engine mints a `kratos/<name>`
