...
impl ProcessRunner for SystemProcessRunner {
    async fn run(&self, request: ProcessRequest) -> Result<ProcessOutput, ProcessRunError> {
        ...
        if request.program == "gh" {
            kratos_harness::compose_login_shell_path(&mut command);
        }
        #[cfg(windows)]
        {
    ...
    fn source(branch: &str, owner: &str, default_branch: Option<&str>) -> CheckoutSourceContext {
        CheckoutSourceContext {
            ...
            branch: BranchHeadContext::resolve(
                ...
                Some("origin"),
                Some(&format!("https://github.com/{owner}/kratos.git")),
            ),
...
printf '%s\n' '[{"number":90,"title":"Host-resolved pull request","url":"https://github.com/acme/kratos/pull/90","state":"OPEN","baseRefName":"main","headRefName":"feature/status","updatedAt":"2026-08-15T12:00:00Z","isCrossRepository":false,"headRepositoryOwner":{"login":"acme"}}]'
"##;
        #[cfg(windows)]
...
[Console]::Out.WriteLine('[{"number":90,"title":"Host-resolved pull request","url":"https://github.com/acme/kratos/pull/90","state":"OPEN","baseRefName":"main","headRefName":"feature/status","updatedAt":"2026-08-15T12:00:00Z","isCrossRepository":false,"headRepositoryOwner":{"login":"acme"}}]')
"##;
    ...
    async fn git_inspector_uses_origin_without_upstream_in_a_multi_remote_checkout() {
        ...
        run_git(
            ...
            &[
                ...
                "add",
                "origin",
                "git@github.com:contributor/kratos.git",
            ],
        );
        run_git(
            &checkout,
        ...
        assert_eq!(
            ...
            Some("git@github.com:contributor/kratos.git")
        );
    ...
    async fn git_inspector_never_runs_remote_transport_for_an_absent_default_branch() {
        ...
        run_git(
            &checkout,
            &["remote", "add", "origin", "git@github.com:acme/kratos.git"],
        );
    ...
    async fn git_inspector_reads_branch_upstream_remote_and_default_branch() {
        let runner = FakeProcessRunner::with_responses([
            ...
            command_success("git@github.com:contributor/kratos.git\n"),
    ...
    fn fork_remote_owner_selector_has_priority() {
        ...
        assert_eq!(context.repository.as_deref(), Some("kratos"));
    ...
    fn matching_local_and_upstream_branches_are_deduplicated() {
        let context = BranchHeadContext::resolve(
            ...
            Some("origin"),
            Some("https://github.com/acme/kratos"),
        );
