...
impl WorkspaceFiles {
    ...
    pub async fn read_image(
        &self,
        request: kratos_proto::ReadWorkspaceImageRequest,
    ) -> Result<kratos_proto::WorkspaceImageChunk, WorkspaceFilesError> {
        let workspace = self.resolve_target(&request.target).await?;
...
fn is_internal_temp_wire_path(path: &str) -> bool {
    path.rsplit('/')
        .next()
        .is_some_and(|name| name.starts_with(".kratos-save-") && name.ends_with(".tmp"))
}
...
fn read_image_blocking(
    ...
    let end = request
        ...
        .min(bytes.len());
    Ok(kratos_proto::WorkspaceImageChunk {
...
fn write_file_blocking(
    ...
    let temp_path = parent.join(format!(".kratos-save-{}.tmp", uuid::Uuid::new_v4()));
...
mod tests {
    ...
    fn watch_normalizes_renames_deduplicates_and_filters_git() {
        ...
        let events = vec![
            ...
            Ok(
                notify::Event::new(EventKind::Modify(ModifyKind::Name(RenameMode::Both)))
                    .add_path(root.join(".kratos-save-dead.tmp"))
...
mod image_tests {
    use super::*;
    use kratos_proto::{ReadWorkspaceImageRequest, WORKSPACE_IMAGE_CHUNK_BYTES, WorkspaceTarget};
    ...
    fn workspace_images_reject_large_files_and_symlinks() {
        ...
        file.set_len(kratos_proto::MAX_WORKSPACE_IMAGE_BYTES as u64 + 1)
            .unwrap();
        assert!(
            read_image_blocking(
                &root,
