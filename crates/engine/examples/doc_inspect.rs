...
//! beyond sqlite's own).
//!
//!   cargo run -p kratos-engine --example doc_inspect -- <store-root> <chat-id>

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut args = std::env::args().skip(1);
    let root = args.next().expect("store root");
    let chat_id = args.next().expect("chat id");
    let store = kratos_sync::DocsStore::open(std::path::PathBuf::from(root))?;
    ...
    raw.import(&bytes)?;
    let doc = kratos_doc::SessionDoc::from_doc(raw);
