...
//!   session id as `RunRequest.resume` (engine-owned, kratos sessions.ts:736);
//! - a kill -9 style crash recovers the session id from the run journal
//!   (kratos recoverDraft, sessions.ts:538-552) and stamps streaming entries
//!   `aborted`;
...
async fn persistent_session_serves_multiple_turns_on_one_child() {
    ...
    // The session PARKS (kratos runsBySession): the second message routes into
...
async fn fresh_crash_auto_resumes_and_notes_the_interruption() {
    ...
    // The run is PICKED BACK UP without any user action (kratos: "not just
...
/// Run with: `cargo test -p kratos-engine --test restart_resume -- --ignored`
