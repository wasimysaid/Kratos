...
use kratos_proto::{GitHistoryRefKind, TerminalEvent};
...
async fn repos_round_trip_add_branches_worktrees() {
    ...
    // Worktree add: kratos/<name> branch, isolated dir under the test root.
    let worktree = repos
        .create_worktree(&repo_dir, "main")
        .await
        .expect("worktree");
    assert!(
        worktree.branch.starts_with("kratos/"),
        "branch: {}",
        worktree.branch
    );
    ...
    // worktree's kratos/<name> branch maps to its linked-checkout path, and
    ...
    // Delete: dir removed, kratos branch removed, refs pruned.
    repos
    ...
    assert!(
        ...
        "kratos branch deleted: {branches:?}"
    );
...
async fn diff_file_text_returns_both_checked_sources() {
    ...
    let escape = kratos_proto::DiffFileSummary {
...
async fn diff_capture_truncates_at_patch_cap() {
    ...
    assert!(snapshot.patch.contains("# Kratos diff truncated"));
}
...
async fn rpc_dispatch_for_m5_methods() {
    ...
    unsafe { std::env::set_var("KRATOS_WORKTREES_DIR", tmp.path().join("worktrees")) };
    let core = assemble(&tmp.path().join("data"));
    let client = kratos_rpc::memory_client(core.rpc_service());
