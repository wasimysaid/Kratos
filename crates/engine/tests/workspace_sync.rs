//! Integration coverage for two `EngineCore`s with distinct data directories and
//! device identities sharing one workspace document. The in-memory bridge
//! cross-imports Loro updates between their workspace docs on a timer, exercising
//! the same optimistic replica behavior used by the durable peer transport.
...
/// The in-process room speaks the durable peer's JSON WebSocket protocol, with
/// both engines' hosts wired to it through the test seam.
async fn bridge(
    a: &EngineCore,
    b: &EngineCore,
