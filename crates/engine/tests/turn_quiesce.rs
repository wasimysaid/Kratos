...
//!    existed in the agent's session data, was absent from kratos's doc).
...
use kratos_doc::{MessagePart, MessageRole, MessageStatus, SessionMessageEntry};
use kratos_engine::{EngineCore, HarnessRegistry, SteerOutcome};
...
fn init_quiesce_env() {
    ...
    ONCE.call_once(|| {
        ...
        unsafe { std::env::set_var("KRATOS_TURN_QUIESCE_MS", QUIESCE_MS.to_string()) };
    });
}
