...
//! relay RPC), runs there, and stamps the chat row's cwd + `kratos/<name>`
...
async fn run_with_worktree_spec_materializes_on_host_and_reuses() {
    ...
    unsafe { std::env::set_var("KRATOS_WORKTREES_DIR", &worktrees_root) };
    ...
    // with the actual kratos/<name> (the composer only knew the base).
    let chat = core
        .workspace
        .chat(CHAT)
