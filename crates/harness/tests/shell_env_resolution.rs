...
use kratos_harness::{AcpHarness, Harness as _};
...
async fn cli_on_login_shell_path_only_is_resolved() {
    ...
    let snapshot = kratos_harness::shell_env::login_shell_path().expect("snapshot captured");
    ...
    assert!(
        kratos_harness::ClaudeHarness::new().installed(),
