...
//!   auto-allow (kratos sessions run unattended, parity with the ACP
...
impl ClaudeHarness {
    ...
    async fn discover_commands(&self) -> Result<Vec<SlashCommand>, HarnessError> {
        ...
        const PROBE_ID: &str = "kratos-command-probe";
        let discovery = async {
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::debug!(target: "kratos_harness::claude", "stderr: {line}");
                    tail.push(&line);
                }
            });
        }
...
async fn load_image_blocks(paths: &[String]) -> Vec<wire::ImageBlock> {
    ...
    for path in paths {
        let bytes = match tokio::fs::read(path).await {
            Ok(bytes) => bytes,
            Err(err) => {
                tracing::warn!(target: "kratos_harness::claude", %path, error = %err, "attachment unreadable; path ref only");
                continue;
            }
        };
        if bytes.len() as u64 > MAX_INLINE_IMAGE_BYTES {
            tracing::debug!(target: "kratos_harness::claude", %path, "attachment over inline cap; path ref only");
            continue;
        }
        let Some(media_type) = image_media_type(std::path::Path::new(path), &bytes) else {
            tracing::debug!(target: "kratos_harness::claude", %path, "attachment not an inline-supported image; path ref only");
            continue;
        };
        ...
        match msg {
            StdinMsg::Line(line) => {
                ...
                if let Err(e) = write.await {
                    tracing::debug!(target: "kratos_harness::claude", "stdin write failed (tolerated): {e}");
                    return;
                }
            }
            StdinMsg::Close => {
            ...
            line = stdout_lines.next_line() => match line {
                Ok(Some(line)) => {
                    ...
                    let frame = match wire::parse_frame(line) {
                        Ok(frame) => frame,
                        Err(e) => {
                            tracing::debug!(target: "kratos_harness::claude", "unparseable frame (skipped): {e}");
                            continue;
                        }
                    };
