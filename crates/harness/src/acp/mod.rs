...
pub struct AcpHarness {
    ...
    /// Mimir enriches the selected row with dependent model traits.
    model_details: tokio::sync::Mutex<std::collections::HashMap<String, Model>>,
    /// Coalesce concurrent picker/title probes.
            ...
            .get_with_timeout(
                ...
                || async {
                    if self.id() == HarnessId::Devin {
                        ...
                        self.discover_models(None).await
                    }
                },
            )
            .await
...
async fn request_draining(
    ...
    res.map(|mut response| {
        let id = response
            ...
            .map(str::to_owned);
        if let Some(mut state) = id.as_deref().and_then(|id| state_updates.remove(id)) {
            // A setter's response is authoritative; do not let a queued older
            // config snapshot undo its acknowledged selection.
...
async fn run_session(session: Session) {
    ...
    let setup = async {
        ...
        // Resolve the requested model through the surface this agent exposes.
    ...
    'main: loop {
        tokio::select! {
            ...
            res = async { turn.as_mut().expect("guarded by if").await }, if turn.is_some() && control_call.is_none() => {
                ...
                let (status, mut error) = prompt_outcome(&event_tx, &res, interrupted).await;
