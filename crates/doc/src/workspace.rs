//! Workspace doc schema over `loro` — the per-org entity index that replaces kratos's
...
//! sets from any device — matching kratos's Mutate surface. Presence rides the room's
//! `EphemeralStore` under keys `presence/{deviceId}` (an online timestamp), replacing
//! kratos's 15s heartbeat writes so liveness never grows the oplog.
//!
//! Timestamps are stored as epoch millis (the session-doc convention) and surface as
//! `chrono::DateTime<Utc>` through the `kratos_proto` entity types.

use chrono::{DateTime, Utc};
use loro::{ExportMode, LoroDoc, LoroMap, LoroValue, ToJson};
use serde::{Deserialize, Serialize};

use kratos_proto::{Chat, ChatConfig, Device, Session, SessionStatus, Space};
...
impl WorkspaceDoc {
    ...
    /// latest run and the cwd it was created under (kratos stored the same pair
...
pub(crate) struct RawChat {
    ...
    checkout_id: Option<String>,
    #[serde(default)]
    source_context: Option<kratos_proto::ConversationSourceContext>,
...
pub(crate) struct RawSession {
    #[serde(default)]
    goal: Option<kratos_proto::GoalState>,
    #[serde(default)]
    goal_control: bool,
    #[serde(default)]
...
mod tests {
    use super::*;
    use kratos_proto::{HarnessId, SandboxLevel};
    ...
    fn conversation_source_context_round_trips_with_legacy_fields() {
        let ws = WorkspaceDoc::new();
        ws.upsert_chat(&chat("chat-1", "dev-a")).unwrap();
        let context = kratos_proto::ConversationSourceContext {
